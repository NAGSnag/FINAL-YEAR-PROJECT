...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt->bind_param("ssssssssss", $username, $email, $deal_amount, $location, $deadline, $job_title, $description, $resume_blob, $banner_blob, $exp,$skill);
    ...
    if ($stmt->execute()) {
        ...
        $stmt_explore = $conn->prepare("INSERT INTO explore (id, name, banner, type, rating, description,email) VALUES (?, ?, ?, ?, ?, ?,?)");
        if (!$stmt_explore) {
        ...
        $stmt_explore->bind_param("issssss", $last_id, $username, $banner_blob, $type, $rating, $job_title,$email);
